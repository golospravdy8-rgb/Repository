...
export default function RucheekGameCanvas({ isVisible, userName = "", userPhone = "", gameRoomId = "general" }: RucheekGameCanvasProps) {
  ...
  useEffect(() => {
    ...
    function draw(dt: number) {
      ...
      remotePlayersRef.current.forEach((rp: any, rpKey: string) => {
        ...
        // Check by playerId instead of name (more reliable)
        const isLocalPlayer = rpKey === playerIdRef.current ||
          rpKey.startsWith(playerIdRef.current + '_');
        if (isLocalPlayer) {
          return;
        }
  ...
  const handleAddPlayer = async () => {
    ...
    try {
      ...
      if (!socketId) {
        console.warn('[ADD-PLAYER] Socket not ready, retrying in 300ms');
        setTimeout(() => {
          try {
            const retrySocketId = pusherRef.current?.connection?.socket_id;
            if (!retrySocketId) {
              console.warn('[ADD-PLAYER] Socket still not ready after retry');
              return;
            }
            fetch('/api/pusher/join', {
              ...
              body: JSON.stringify({
                ...
                socket_id: retrySocketId,
              })
            }).catch(e => console.warn('[ADD-PLAYER] Retry failed:', e));
          } catch (e) {
            console.warn('[ADD-PLAYER] Retry error:', e);
          }
        }, 300);
        return;
      }
