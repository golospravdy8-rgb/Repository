  ...
  useEffect(() => {
    ...
    function stepBall(b: any, dt_normalized: number) {
      ...
      while (b._accumulator >= FIXED_DT && b.state === 'flying') {
        ...
        // участвуют в коллизии — это дужки слева и справа.
        // Точки сверху/снизу эллипса НЕ блокируют мяч (он проходит).

        if (b.rimHitTimer === undefined) b.rimHitTimer = 0;
        if (b.rimContacts === undefined) b.rimContacts = 0;
        if (b.hitBoard === undefined) b.hitBoard = false;
        if (b.rimHitTimer > 0) b.rimHitTimer--;

        // TUBE_R из диагностики = HOOP_R * 0.22 ≈ 5.94px
        const RIM_TUBE_R = HOOP_R * 0.22;
        const BALL_R = BALL_RADIUS;

        // Проверяем только если мяч рядом по высоте
        // Диапазон из Basketball Lab: BALL_RADIUS * 1.05 от уровня дужки
        const rimYdist = Math.abs(b.y - HOOP_Y);
        // ── ГЛОБАЛЬНЫЙ COOLDOWN: пропускаем коллизии пока rimHitTimer активен ──
        if (rimYdist < BALL_R + RIM_TUBE_R * 2.5 && b.rimHitTimer === 0) {
          ...
          for (let i = 0; i < 16; i++) {
            ...
            // КЛЮЧЕВОЕ из Basketball Lab:
            // Только боковые точки кольца являются дужками.
            // Точки сверху/снизу — это внутри кольца, мяч там проходит.
            // Пропускаем точки где |cosA| < 0.25 (верх и низ эллипса)
            if (Math.abs(cosA) < 0.25) continue;

            // Координаты точки на окружности кольца
            // RIM_RX = HOOP_R (горизонталь), RIM_RY = HOOP_R*0.32 (вертикаль)
            const rimPointX = HOOP_X + cosA * HOOP_R;
            const rimPointY = HOOP_Y + sinA * HOOP_R * 0.32;

            const dx = b.x - rimPointX;
            const dy = b.y - rimPointY;
            const dist = Math.sqrt(dx * dx + dy * dy);

            if (dist < BALL_R + RIM_TUBE_R && dist > 0.001) {
              const nx = dx / dist;
              const ny = dy / dist;
              const velocityNormal = b.vx * nx + b.vy * ny;

              if (velocityNormal < 0) {
                const impactSpeed = Math.sqrt(b.vx*b.vx + b.vy*b.vy);
                const spin = b.spin || b.angularVel || b.omega || 0;

                // ── BASKETBALL LAB коэффициенты ──────────────────
                // Источник: g.ballVel.addScaledVector(diff, -(1+0.55)*velocityNormal)
                // g.ballVel.multiplyScalar(0.68) — но мы поднимаем до 0.82
                // чтобы мяч не зависал (диагностика: 0.68^4 = 21% энергии)
                let RESTITUTION = 0.55;   // из Basketball Lab
                let ENERGY_KEEP = 0.82;   // поднято с 0.68 — иначе мяч зависает

                // Сила удара
                if (impactSpeed > 14) {
                  // Сильный — жёсткий отскок (Вариант 5, задняя → вперёд)
                  RESTITUTION = 0.70;
                  ENERGY_KEEP = 0.88;
                } else if (impactSpeed < 4) {
                  // Слабый — мяч "прилипает" к дужке (Варианты 2, 3)
                  // Basketball Lab: минимум 0.78 чтобы не зависал (0.78^8 = 14.3%)
                  RESTITUTION = 0.42;
                  ENERGY_KEEP = 0.78;
                }
                ...
                // Спин (Варианты 13, 14)
                if (spin < -0.3) {
                  // Обратный спин смягчает отскок
                  RESTITUTION *= 0.65;
                  ENERGY_KEEP *= 0.90;
                } else if (Math.abs(spin) < 0.05) {
                  // Без спина — жёстче
                  RESTITUTION *= 1.15;
                }

                // Парашютный бросок угол > 70° (Вариант 15)
                const entryAngle = Math.abs(
                  Math.atan2(-b.vy, Math.abs(b.vx)) * 180 / Math.PI
                );
                if (entryAngle > 70) {
                  RESTITUTION *= 0.55;
                  ENERGY_KEEP *= 0.85;
                ...
                // ── Применить отскок (Basketball Lab формула) ────
                b.vx += -(1 + RESTITUTION) * velocityNormal * nx;
                b.vy += -(1 + RESTITUTION) * velocityNormal * ny;
                b.vx *= ENERGY_KEEP;
                b.vy *= ENERGY_KEEP;

                // Penetration fix — вытолкнуть мяч из дужки
                // Basketball Lab: просто копирует позицию без extra offset
                const overlap = (BALL_R + RIM_TUBE_R) - dist;
                b.x += nx * overlap * 1.02; // 2% запас чтобы не застрять
                b.y += ny * overlap * 1.02;

                // Счётчик касаний и таймер
                b.rimContacts = (b.rimContacts || 0) + 1;
                b.rimHitTimer = 18; // 0.3 сек при 60fps

                // Rimroll — притяжение к центру 2+ касания (Вариант 8)
                if (b.rimContacts >= 2) {
                  b.vx += (HOOP_X - b.x) * 0.015;
                }

                // Танцует на кольце 3+ касания (Вариант 7)
                if (b.rimContacts >= 3 && impactSpeed < 5) {
                  if (Math.random() < 0.55) {
                    b.vx = (HOOP_X - b.x) * 0.10;
                    b.vy = Math.abs(b.vy) * 0.25 + 0.6 * scaleY;
                  }
                }

                // ── АВАРИЙНЫЙ ВЫХОД если мяч завис (> 8 касаний) ──
                // Basketball Lab: g.rimHits cooldown + force escape
                if (b.rimContacts > 8) {
                  // Принудительно выбросить мяч из кольца
                  const escapeDir = b.x < HOOP_X ? -1 : 1;
                  b.vx = escapeDir * 3 * scaleX;
                  b.vy = -2 * scaleY;
                  b.rimContacts = 0;
                  b.rimHitTimer = 30; // заморозить коллизии на 0.5 сек
                }
