...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!token) {
      console.error(`[marketplace-upload ${uploadId}] LOGOS_READ_WRITE_TOKEN not found`);
    ...
    try {
      ...
      console.log(`[marketplace-upload ${uploadId}] ✅ Success: ${blob.url} (${Date.now() - startTime}ms)`);
      return NextResponse.json({ url: blob.url });
    } catch (blobErr) {
      const errMsg = blobErr instanceof Error ? blobErr.message : String(blobErr);
      console.error(`[marketplace-upload ${uploadId}] Vercel Blob error: ${errMsg}`);
